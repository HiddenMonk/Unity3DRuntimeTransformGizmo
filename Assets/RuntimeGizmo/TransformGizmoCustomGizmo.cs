...
namespace RuntimeGizmos
{
	public class TransformGizmoCustomGizmo : MonoBehaviour
	{
		...
		void CheckForSelectedAxis()
		{
			...
			if(Input.GetMouseButtonDown(0))
			{
				...
				if(Physics.Raycast(transformGizmo.myCamera.ScreenPointToRay(Input.mousePosition), out hitInfo, Mathf.Infinity, mask))
				{
					...
					if(transformGizmo.type == TransformType.Move) selectedAxis = customTranslationGizmos.GetSelectedAxis(hitInfo.collider);
					else if(transformGizmo.type == TransformType.Rotate) selectedAxis = customRotationGizmos.GetSelectedAxis(hitInfo.collider);
					else if(transformGizmo.type == TransformType.Scale) selectedAxis = customScaleGizmos.GetSelectedAxis(hitInfo.collider);

					transformGizmo.SetTranslatingAxis(selectedAxis);
				}
			}
		}
		...
		{
			if(transformGizmo.type == TransformType.Move) DrawCustomGizmo(customTranslationGizmos);
			else if(transformGizmo.type == TransformType.Rotate) DrawCustomGizmo(customRotationGizmos);
			else if(transformGizmo.type == TransformType.Scale) DrawCustomGizmo(customScaleGizmos);
		}
		...
		void DrawCustomGizmo(CustomTransformGizmos customGizmo)
		{
			...
			if(transformGizmo.type == TransformType.Scale)
			{
		...
		void ShowProperGizmoType()
		{
			...
			customTranslationGizmos.SetEnable(hasSelection && transformGizmo.type == TransformType.Move);
			customRotationGizmos.SetEnable(hasSelection && transformGizmo.type == TransformType.Rotate);
			customScaleGizmos.SetEnable(hasSelection && transformGizmo.type == TransformType.Scale);
		}
	}
